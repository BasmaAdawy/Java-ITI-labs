...
public class YoutubeTitleWordCount {
    ...
    public static void main(String[] args) throws IOException {
       ...
       // JavaRDD<String>
        ...
        JavaRDD<String> tags = videos
                ...
                .filter (StringUtils::isNotBlank);
       // JavaRDD<String>
        JavaRDD<String> twords = tags.flatMap (tag -> Arrays.asList (tag
                .toLowerCase ()
                .trim ()
                .replaceAll ("\\p{Punct}", " ")
               .split (" ")).iterator ());
        System.out.println(words.toString ());
        // COUNTING
        Map<String, Long> twordCounts = twords.countByValue ();
        List<Map.Entry> tsorted = wordCounts.entrySet ().stream ()
                .sorted (Map.Entry.comparingByValue ()).collect (Collectors.toList ());
        // DISPLAY
        for (Map.Entry<String, Long> entry : tsorted) {
            System.out.println (entry.getKey () + " : " + entry.getValue ());
        }
        //LocalTime tend= LocalTime.now ();
    ...
    public static String extractTitle(String videoLine) {
        try {
            return videoLine.split (COMMA_DELIMITER)[2];      //  2 is for titles column
    ...
    public static String extractTags(String videoLine) {
        try {
            return videoLine.split (COMMA_DELIMITER)[6];      // 6 is for tags column
        } catch (ArrayIndexOutOfBoundsException e) {
            return "";
        }
    }
